function zdez=bilard(fis1,fis2)
    ...
    czasZderzenia=0.05;
    polozenie=[22 12; 22.6 57; 40 70; 52 38];    %współrzędne x,y bil
    ...
    wtt=0.05;         %współczynnik tarcia tocznego
    mb=0.1;         %masa bili
    ...
    while true
        ...
        %redukcja prędkości poprzez tarcie toczne 
        for i=1:n
            stan_tarcia=[];
            stan_tarcia=[predkosc(i,1),predkosc(i,2)];
            tarcie=evalfis(t, stan_tarcia);
            tarcie(1)=tarcie(1)+tarcie(1)/abs(tarcie(1))*wtt;
            tarcie(2)=tarcie(2)+tarcie(2)/abs(tarcie(2))*wtt;
            if abs(predkosc(i,1))>abs(tarcie(1))
                predkosc(i,1)=predkosc(i,1)-tarcie(1);
            else
                predkosc(i,1)=0;
            end
            if abs(predkosc(i,2))>abs(tarcie(2))
                predkosc(i,2)=predkosc(i,2)-tarcie(2);
            else
                predkosc(i,2)=0;
            end
        end
